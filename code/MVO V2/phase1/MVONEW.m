function [path,costValue] = MVONEW(map, start, goal)
...
i = 1;
while i<=nUni
    [paths,CurrentNode,NVNodes] = getPath(CollisionTest,StartNode,GoalNode,AllPoints(:,:),nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),true);
    %fprintf('%d %d\n',curNodeIndex, nodes);
        ...
        Back_hole_index=i;
            ...
            if r1<normalized_sorted_cost_func(i)
                ...
                if r2<WEP_C
                    ...
                    [newPath,Cost] = changePath(current(:,4),toward(:,4),AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),true);
...
%                     end
                ...
                elseif r2>=WEP_C
                    [newPath,Cost] = changePath(universes(Back_hole_index).Position(:,4),[],AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),true);
                    newPath = removeLoops(newPath);  
                    universes(Back_hole_index).Position = AllPoints(newPath(:,1),:);
                    universes(Back_hole_index).Position(:,4) = newPath(:,1);
                    universes(Back_hole_index).Cost = Cost;
